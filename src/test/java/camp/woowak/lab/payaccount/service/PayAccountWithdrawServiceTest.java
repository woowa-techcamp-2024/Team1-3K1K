...
class PayAccountWithdrawServiceTest {
	@Autowired
	private PayAccountRepository payAccountRepository;

	@Autowired
	private PayAccountWithdrawService payAccountWithdrawService;

	private PayAccount payAccount;
	private long originBalance = 1000L;
	...
	class WithdrawAccount {
		...
		void withdrawAccount() {
			...
			//when
			long afterBalance = payAccountWithdrawService.withdrawAccount(command);
		...
		void withdrawAccountNotFound() {
			...
			//when & then
			assertThatThrownBy(() -> payAccountWithdrawService.withdrawAccount(command))
